...
shapleyImportance = function(object, data, target, features, measures,
  m = "all.unique", n.feat.perm = 50, local = FALSE) {
  ...
  mc = rbindlist(mc, idcol = "features")
...
marginalContributions = function(object, data, f, measures, perm,
  ...
  imp.without.f = featureImportance(object = object, data = data,
    features = set.without.f[unique.ind],
    measures = measures, n.feat.perm = n.feat.perm, local = local)
  imp.with.f = featureImportance(object = object, data = data,
...
calculateValueFunction = function(features, object, data, measures,
  n.feat.perm = 50, local = FALSE) {
  mid = BBmisc::vcapply(measures, function(x) x$id)
  ret = featureImportance(object = object, data = data, features = features,
    measures = measures, n.feat.perm = n.feat.perm, local = local)
...
#   })
#   # FIXME: To speedup, compute featureImportance for set = unique(lapply(unname(Reduce(c, set)), sort))
#   shap = lapply(set, function(x) {
#     with.f = x$with.f
#     without.f = x$without.f
#     imp.with.f = featureImportance(object, data = data, features = list(with.f),
#       measures = measures, n.feat.perm = n.feat.perm, local = local)
#     imp.with.f = imp.with.f[, lapply(.SD, mean), .SDcols = mid] # by = "features"
#
#     if (length(without.f) != 0) {
#       imp.without.f = featureImportance(object, data = data, features = list(without.f),
#         measures = measures, n.feat.perm = n.feat.perm, local = local)
...
# shapley = function(f, m, object, data, measures, n.feat.perm = 50, local = FALSE, features, remove.duplicates = FALSE) {
#   set = chooseFeatures(f, features, m, remove.duplicates = remove.duplicates)
#
#   imp.with.f = featureImportance(object, data = data, features = set$with.f, measures = measures, n.feat.perm = n.feat.perm, local = local)
#   imp.without.f = featureImportance(object, data = data, features = set$without.f, measures = measures, n.feat.perm = n.feat.perm, local = local)
